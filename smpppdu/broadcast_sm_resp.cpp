...
{
  //--------------------------------------------------------------------------------
  PDU_broadcast_sm_resp::PDU_broadcast_sm_resp() :
    SMPP_PDU(CommandLength(MinLength), CommandId(CommandId::BroadcastSmResp), CommandStatus(CommandStatus::ESME_ROK), SequenceNumber::Min)
  {}

  //--------------------------------------------------------------------------------
  PDU_broadcast_sm_resp::PDU_broadcast_sm_resp(const char* b) :
    SMPP_PDU(b)
  {
    decode(b);
  }
  ...
  //--------------------------------------------------------------------------------
  PDU_broadcast_sm_resp::PDU_broadcast_sm_resp(const PDU_broadcast_sm_resp &o) :
  ...
  //--------------------------------------------------------------------------------
  unsigned int PDU_broadcast_sm_resp::getBodyLength()
  {
    return message_id_.length();
  }

  //--------------------------------------------------------------------------------
  std::string PDU_broadcast_sm_resp::encodeStandardParameters()
  {
    return (std::string)message_id_;
  }

  //--------------------------------------------------------------------------------
  uint8_t *PDU_broadcast_sm_resp::decodeStandardParameters(const uint8_t *buff)
  {
