...
{
  //--------------------------------------------------------------------------------
  PDU_submit_sm_resp::PDU_submit_sm_resp() :
    SMPP_PDU(CommandLength(MinLength), CommandId(CommandId::SubmitSmResp), CommandStatus(CommandStatus::ESME_ROK), SequenceNumber::Min) {}

  //--------------------------------------------------------------------------------
  PDU_submit_sm_resp::PDU_submit_sm_resp(const char* b) :
    SMPP_PDU(b)
  {
    decode(b);
  }
  ...
  //--------------------------------------------------------------------------------
  PDU_submit_sm_resp::PDU_submit_sm_resp(const PDU_submit_sm_resp& o) :
  ...
  //--------------------------------------------------------------------------------
  unsigned int PDU_submit_sm_resp::getBodyLength()
  {
    return message_id_.length();
  }

  //--------------------------------------------------------------------------------
  std::string PDU_submit_sm_resp::encodeStandardParameters()
  {
    return (std::string)message_id_;
  }

  //--------------------------------------------------------------------------------
  uint8_t *PDU_submit_sm_resp::decodeStandardParameters(const uint8_t *buff)
  {
